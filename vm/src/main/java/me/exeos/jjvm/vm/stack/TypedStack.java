...
public class TypedStack {
    ...
    public Object popJVMType(ConstantPool cp, Heap heap) {
        ...
        switch (type) {
            ...
            default -> throw new IllegalStateException("Invalid stack type: " + type);
        }
    }

    public void dup(int factor) {
        if (stackEntryPointer == 0) {
            throw new IllegalStateException("Can't dup empty stack");
        }
    ...
    public void ensureMinHeight(int min) {
        if (stackEntryPointer < min) {
            throw new IllegalStateException("Invalid stack height.");
        }
    }

    public void ensureHeight(int height) {
        if (stackEntryPointer != height) {
            throw new IllegalStateException("Invalid stack height.");
        }
    }
}
